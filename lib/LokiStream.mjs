...
export class LokiStream extends Writable {
  constructor(endPoint, opt = {}) {
    ...
    // Setting HWM to maxCount so we can handle another maxCount
    // messages while we're flushing a batch.
    super({ highWaterMark: options.maxCount, objectMode: true });
  ...
  _startTicker() {
    this.ticker = setInterval(() => {
      ...
      if (!this.write(heartbeat)) {
        // We're full so stop sending heartbeats until we've drained
        this._stopTicker();
        once(this, "drain").then(() => this._startTicker());
      }
    }, Math.max(this.opt.maxAge / 10, 10));
  }
